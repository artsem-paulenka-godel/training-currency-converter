...
Reference: https://github.com/github/awesome-copilot/blob/main/instructions/a11y.instructions.md
...
Since this project uses Tailwind CSS, use the built-in `sr-only` and `not-sr-only` utilities instead of a custom CSS class:
...
If Tailwind is not available, use a custom `.visually-hidden` class (avoid naming it `.sr-only` to prevent clashing with Tailwind's utility):
